...
class WorkflowQueueTests(unittest.TestCase):
    ...
    def test_train_workflow_manual_policy_preserves_auto_seed_without_collision_iteration(self) -> None:
        controller = _FakeQueueRunController()
        with tempfile.TemporaryDirectory() as tmpdir:
            config = dict(DEFAULT_CONFIG_VALUES)
            config["lerobot_dir"] = tmpdir
            ...
            with patch("robot_pipeline_app.workflow_queue.build_train_request_and_command", side_effect=fake_build_train_request_and_command):
                ok, _message = queue.enqueue(item)

            self.assertTrue(ok)
            self.assertEqual(captured["job_name"], "demo_train_act_1")

    def test_train_workflow_always_policy_iterates_manual_job_name(self) -> None:
        controller = _FakeQueueRunController()
        with tempfile.TemporaryDirectory() as tmpdir:
            config = dict(DEFAULT_CONFIG_VALUES)
            config["lerobot_dir"] = tmpdir
            ...
            trained_models_dir = Path(tmpdir) / "trained_models"
            occupied = trained_models_dir / "custom_run_9"
            occupied.mkdir(parents=True, exist_ok=True)
            (occupied / "checkpoint.txt").write_text("occupied\n", encoding="utf-8")
            queue = WorkflowQueueService(config=config, run_controller=controller, append_log=lambda _line: None)
            item = build_train_sim_eval_queue_item(
                ...
                train_form_values={
                    ...
                    "output_dir": str(trained_models_dir),
                    "device": "cuda",
                    "job_name": "custom_run_9",
                },
                train_job_name_state={"value": "custom_run_9", "mode": "manual"},
                sim_eval_settings={"episodes": "5", "output_dir": str(Path(tmpdir) / "sim_eval")},
            )
            ...
            self.assertTrue(ok)
            self.assertEqual(captured["job_name"], "custom_run_10")
    ...
    def test_train_deploy_eval_manual_policy_preserves_auto_seed_without_iteration(self) -> None:
        controller = _FakeQueueRunController()
        with tempfile.TemporaryDirectory() as tmpdir:
            config = dict(DEFAULT_CONFIG_VALUES)
            config["lerobot_dir"] = tmpdir
            ...
            def fake_build_deploy_request_and_command(*, config, deploy_root_raw, deploy_model_raw, eval_dataset_raw, eval_episodes_raw, eval_duration_raw, eval_task_raw, target_hz_raw):  # type: ignore[no-untyped-def]
                ...
                return request, ["python3", "-m", "lerobot.deploy"], {"last_eval_dataset_name": "eval_run_1"}, None
            ...
            self.assertEqual(captured["eval_dataset_raw"], "alice/eval_run_1")
