...
def setup_visualizer_tab(*, root: Any, visualizer_tab: Any, config: dict[str, Any], colors: dict[str, str], log_panel: Any, messagebox: Any, background_jobs: UiBackgroundJobs | None = None) -> VisualizerTabHandles:
    ...
    bind_yview_wheel_scroll(source_list)
    ...
    bind_yview_wheel_scroll(insights_tree)

    videos_title = ttk.Label(right, text="Video Feed", style="SectionTitle.TLabel")
    videos_title.grid(row=4, column=0, sticky="w")
    video_tree = ttk.Treeview(right, columns=("file", "size"), show="headings", height=8, style="History.Treeview")
    ...
    bind_yview_wheel_scroll(video_tree)

    action_row = ttk.Frame(right, style="Panel.TFrame")
    action_row.grid(row=6, column=0, sticky="w", pady=(8, 0))
    ttk.Button(action_row, text="Open Source Folder", command=lambda: _open_selected_source()).pack(side="left")
    ttk.Button(action_row, text="Open Selected Video", command=lambda: _open_selected_video()).pack(side="left", padx=(6, 0))
