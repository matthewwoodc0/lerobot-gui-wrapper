...
def _probe_record_help_flags(config: dict[str, Any], record_entrypoint: str) -> tuple[set[str], str]:
    ...
    variants = (
        [sys.executable, "-m", record_entrypoint, "--help"],
        [sys.executable, "-m", record_entrypoint, "-h"],
    )
...
def _cache_key(config: dict[str, Any], include_flag_probe: bool) -> tuple[str, ...]:
    return (
        sys.executable,
        str(sys.platform),
...
def probe_lerobot_capabilities(
    ...
    *,
    ...
    key = _cache_key(config, include_flag_probe)
    ...
    if include_flag_probe:
        ...
        cached_probe_key = _cache_key(config, True)
...
def compatibility_checks(
    ...
    *,
    ...
    policy = str(config.get("compat_policy", "latest_plus_n_minus_1")).strip() or "latest_plus_n_minus_1"
