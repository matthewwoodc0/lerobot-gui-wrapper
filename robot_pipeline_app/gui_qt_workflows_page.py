...
class QtWorkflowsPage(_PageWithOutput):
    ...
    def __init__(
        ...
        *,
        ...
        workflows_service: WorkflowQueueService,
    ) -> None:
        super().__init__(
            title="Workflows",
            subtitle="Chain lightweight local recipes such as record/upload and train/eval follow-ups.",
            append_log=append_log,
        )
        self.config = config
        self._workflows_service = workflows_service
        self._rows: list[dict[str, Any]] = []
        self._workflows_service.add_listener(self.refresh_workflow_views)
        ...
        self.content_layout.addWidget(self._build_workflow_status_card())
    ...
    def _build_record_recipe_card(self):
        ...
        actions = QHBoxLayout()
        enqueue_button = QPushButton("Add Record -> Upload Workflow")
    ...
    def _build_sim_eval_recipe_card(self):
        ...
        actions = QHBoxLayout()
        enqueue_button = QPushButton("Add Train -> Sim Eval Workflow")
        enqueue_button.setObjectName("AccentButton")
    ...
    def _build_deploy_eval_recipe_card(self):
        ...
        actions = QHBoxLayout()
        enqueue_button = QPushButton("Add Train -> Deploy Eval Workflow")
        enqueue_button.setObjectName("AccentButton")
    ...
    def _build_workflow_status_card(self):
        card, layout = _build_card("Workflow Status")
        self.workflow_table = QTableWidget(0, 5)
        _set_table_headers(self.workflow_table, ["ID", "Recipe", "Status", "Step", "Artifacts"])
        _set_readonly_table(self.workflow_table)
        self.workflow_table.itemSelectionChanged.connect(self._on_selection_changed)
    ...
    def refresh_workflow_views(self) -> None:
        self._rows = list(self._workflows_service.snapshots())
        self.workflow_table.setRowCount(len(self._rows))
        for row_index, row in enumerate(self._rows):
            ...
            for col_index, value in enumerate(values):
                self.workflow_table.setItem(row_index, col_index, QTableWidgetItem(str(value)))
        if self._rows and self.workflow_table.currentRow() < 0:
            self.workflow_table.selectRow(0)
        elif not self._rows:
            self.output_card.show()
            self._set_output(title="Workflows", text="No workflows yet.", log_message=None)
    ...
    def _on_selection_changed(self) -> None:
        ...
        self.output_card.show()
        if row is None:
            self._set_output(title="Workflows", text="Select a workflow to inspect it.", log_message=None)
            return
        payload = {
    ...
    def cancel_active_workflow(self) -> None:
        ok, message = self._workflows_service.cancel_active()
    ...
    def resume_pending_workflows(self) -> None:
        ok, message = self._workflows_service.resume_pending()
        self._set_output(title="Workflows Resumed" if ok else "Resume Pending", text=message, log_message=message)

    def retry_interrupted_workflow(self) -> None:
        row = self._selected_row()
        if row is None:
            self._set_output(title="No Selection", text="Select an interrupted workflow first.", log_message="Workflow retry skipped with no selection.")
            return
        queue_id = row.get("queue_id")
        try:
            queue_id = int(queue_id)
        except (TypeError, ValueError):
            self._set_output(title="Retry Failed", text="The selected workflow is missing a valid id.", log_message="Workflow retry failed due to invalid selection.")
            return
        ok, message = self._workflows_service.retry_interrupted_step(queue_id)
    ...
    def clear_finished_interrupted_workflows(self) -> None:
        ok, message = self._workflows_service.clear_finished_interrupted()
        self._set_output(title="Workflows Cleared" if ok else "Clear Workflows", text=message, log_message=message)

    def open_latest_artifact(self) -> None:
        row = self._selected_row()
        if row is None:
            self._set_output(title="No Selection", text="Select a workflow first.", log_message="Workflow artifact open skipped with no selection.")
            return
        artifacts = row.get("artifacts")
        if not isinstance(artifacts, list) or not artifacts:
            self._set_output(title="No Artifact", text="This workflow has not produced any artifacts yet.", log_message="Workflow artifact open skipped with no artifacts.")
            return
        latest = artifacts[-1]
        target = Path(str(latest.get("path", "")).strip())
        ok, message = open_path_in_file_manager(target)
        self._set_output(title="Open Artifact" if ok else "Open Failed", text=str(target if ok else message), log_message="Workflow artifact open requested." if ok else "Workflow artifact open failed.")
